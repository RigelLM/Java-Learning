...
public final class StringReassembly {
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        for (String s : strSet) {
            for (String e : strSet) {
                if (!e.equals(s) && e.contains(s)) {
                    precondition = false;
                }
            }
        }
        assert precondition : "Violation of: CONTAINS_NO_SUBSTRING_PAIRS(strSet)";

        // check whether str is a substring of any element in strSet
        boolean condition = true;
        for (String elements : strSet) {
            if (elements.contains(str)) {
                condition = false;
            }
        }

        // if not, remove all the substrings of str in strSet
        if (condition) {
