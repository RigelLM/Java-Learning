...
public final class PizzaOrderManager {
    ...
    private static void getPriceMap(String fileName,
        ...
        while (!input.atEOS()) {
            ...
            int price = Integer
                    .parseInt(line.substring(pos + 1, line.length()));

            temp.add(word, price);
        }
        priceMap.transferFrom(temp);
        input.close();
    }
    ...
    private static int getOneOrder(SimpleReader input,
        ...
        if (!input.atEOS()) {
            ...
            while (!topping.equals("")) {
                int price = toppingPriceMap.value(topping);
                toppingPrice += price;
    ...
    private static void putPrice(SimpleWriter output, int price) {
        ...
        output.println(dollars + " dollars and " + cents + " cents.");
    }
